...
> 哎哎哎:# t0]https://golangbyexample . com/constant-golang/
...
像任何其他变量一样，如果常量在任何函数范围之外的文件顶部声明，则它在包中是全局的。例如，在下面的程序名称中会有一个全局常数在整个**主**包的任何功能中可用。请注意，常量名称在主包之外不可用。要在主包之外使用，它必须以大写字母开头。

请参见下面的代码。它还显示了包中的局部常数的示例。
