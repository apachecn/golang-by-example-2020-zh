...
Golang的恐慌类似于例外。恐慌是指在异常情况下退出程序。程序中出现死机有两种方式
...
在上面的程序中，我们有一个函数 **checkAndPrint** ，它在参数中传递的索引处检查并打印切片元素。如果传递的索引大于数组的长度，程序就会死机。我们还在函数**的开始处添加了一个名为 **handleOutIfBounds** 的延迟函数**。该函数包含如下的`recover()`函数调用
...
请注意，如果延迟函数和`recover()`函数不是从恐慌函数中调用的，那么在这种情况下，恐慌也可以在被调用的函数中恢复。事实上，在调用栈链中，随后从恐慌中恢复是可能的。
...
golang 的调试包还提供了 StackTrace 函数，可以用来打印`recover()`函数中死机的栈跟踪
...
在上面的程序中，我们使用 **StackTrace** 函数打印`recover()`函数中死机的栈跟踪。从输出中可以看出，它打印了正确的栈跟踪
